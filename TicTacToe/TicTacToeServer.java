...
public class TicTacToeServer extends JFrame {
	...
	public boolean check_win(String ch) {

		System.out.println("CHECK WIN " + ch);
		for (int i = 0; i < 3; ++i) {
			// checa Horizontal
			if (board[i * 3].equals(ch) && board[i * 3 + 1].equals(ch) && board[i * 3 + 2].equals(ch)) {
				//displayMessage("\n" + ch + " VENCEU ");
				return true;
			}
			// checa Vertical
			if (board[i].equals(ch) && board[i + 3].equals(ch) && board[i + 6].equals(ch)) {
				//displayMessage("\n" + ch + " VENCEU ");
				return true;
			}
		...
		// checa Diagonal
		if ((board[0].equals(ch) && board[4].equals(ch) && board[8].equals(ch))
				|| (board[2].equals(ch) && board[4].equals(ch) && board[6].equals(ch))) {
			...
			try {
				...
				while (isGameOver() == 0) {
					...
					if (validateAndMove(location, playerNumber)) {
						...
						if (isGameOver() != 0) {
							if (isGameOver() == 1) {
								output.format("PARABENS FDP, VC GANHOU ESTA MERDA");
								displayMessage("\n" + mark + " VENCEU ");
								output.flush(); // flush output
							} else {
								output.format("DEU VEIA");
								output.flush(); // flush output
								
								if (currentPlayer == 0) {
									players[0].output.format("DEU VEIA");
									players[0].output.flush(); // flush output
								} else {
									players[1].output.format("DEU VEIA");
									players[1].output.flush(); // flush output
								}
							}
